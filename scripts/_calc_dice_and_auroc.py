...
if __name__ == '__main__':
    ...
    tensor_data = torch.load(f"/media/user/ボリューム2/out/diffmap_pt/{SUBJECT_ID}/{SICE_ID}/diff_all.pt")
    ...
    # 入出力差分をOtsu手法により二値化
    thresh_value, binarized_input_image = cv2.threshold(norm_image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    ground_truth_mask = cv2.imread(f"/media/user/ボリューム2/out/abnormal_sample_and_heatmap/{SUBJECT_ID}/{SICE_ID}_label.jpg", cv2.IMREAD_GRAYSCALE)
    ...
    plt.figure(figsize=(8, 6))
    plt.hist(binarized_ground_truth_mask.ravel(), bins=100, color='b', alpha=0.7, rwidth=0.8)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.ylim(0, 1000)
    plt.show()
