...
if __name__ == "__main__":
    subjet_id = "353"
    # slice_id = 0
    save_dir = f"/mnt/ito/diffusion-anomaly/out/binarized_img_000-154/{subjet_id}"
    ...
    diff_voxel = nibabel.load(
        f"/media/user/ボリューム/out/diffmap_and_bin_voxel_000-154/{subjet_id}/diff_voxel.nii"
    )
    diff_voxel_tensor = diff_voxel.get_fdata()
    bin_label_voxel = nibabel.load(
        f"/media/user/ボリューム/out/diffmap_and_bin_voxel_000-154/{subjet_id}/bin_label_voxel.nii"
    )
    bin_label_voxel_tensor = bin_label_voxel.get_fdata()
    otsubin_diff_voxel = nibabel.load(
        f"/media/user/ボリューム/out/diffmap_and_bin_voxel_000-154/{subjet_id}/otsubin_diff_voxel.nii"
    )
