...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(diffmap_dirs):
        if not dirs:
            ...
            ground_truth_mask_path = ground_truth_mask_path.replace(
                "diffmap_pt", "abnormal_sample_and_heatmap"
            )
            ...
            plt.savefig(
                f"{diffmap_dirs}/{subject_path[-1]}/{slice_id}/diffmap_hist_pt.jpg"
            )
            plt.close()
            ...
            thresh_value, binarized_input_image = cv2.threshold(
                norm_image, 0, 255, cv2.THRESH_OTSU
            )
            ...
            plt.axis("off")
            plt.savefig(
                f"{diffmap_dirs}/{subject_path[-1]}/{slice_id}/binarized_diffmap_image.jpg",
                bbox_inches="tight",
                pad_inches=0,
            )
            plt.close()
            ...
            if ground_truth_mask is None:
                ...
                binarized_ground_truth_mask = binarized_ground_truth_mask.astype(
                    np.uint8
                )
                ...
                plt.axis("off")
                plt.savefig(
                    f"{diffmap_dirs}/{subject_path[-1]}/{slice_id}/binarized_label_image.jpg",
                    bbox_inches="tight",
                    pad_inches=0,
                )
                plt.close()
